"""P3-G/J: the local-search glue folds, held bitwise against the path they replace.

Ticket: ``em_parity_tickets_20260918/P3G_local_glue_programs.md``.

Two opt-in changes, each measured by P3-E's census at the local state D of the
...
not a tolerance: neither change touches an arithmetic expression, so
...
from relax.local import local_bucket_stages as lbs
