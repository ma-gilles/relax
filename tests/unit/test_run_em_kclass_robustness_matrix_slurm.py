...
def test_case_jobs_only_verify_setup_sealed_cuda_lib(tmp_path):
    ...
    text = script.read_text()
    assert f"export RELAX_CUDA_LIB={tmp_path}/librecovar_cuda.so" in text
    assert 'setup did not seal the shared CUDA library' in text
    assert 'sha256sum --check "${RELAX_CUDA_LIB}.sha256"' in text
    assert 'make -C recovar/cuda' not in text
    assert 'flock "$(dirname "${RELAX_CUDA_LIB}")/build.lock"' not in text
...
def test_setup_script_allows_external_relion_bind_build_dir(tmp_path):
    ...
    assert 'flock "$(dirname "${RELAX_CUDA_LIB}")/build.lock"' in text
    assert 'make -C relax/cuda LIB="${CUDA_LIB_TMP}" all' in text
    assert 'sha256sum "${RELAX_CUDA_LIB}" > "${RELAX_CUDA_LIB}.sha256"' in text
