...
def test_em_imports_are_sorted_and_used():
    ...
    result = subprocess.run([str(ruff), "check", "relax", "--select", "I001,F401", "--exclude", "*.ipynb"], cwd=ROOT, capture_output=True, text=True)
...
def test_em_scripts_and_tests_bind_every_name():
    ...
    if not ruff.exists():
        pytest.skip("ruff is not installed next to the interpreter")
    result = subprocess.run([str(ruff), "check", "scripts", "tests/unit", "relax", "--select", "F821", "--exclude", "*.ipynb"], cwd=ROOT, capture_output=True, text=True)
    assert result.returncode == 0, result.stdout[-2000:]
