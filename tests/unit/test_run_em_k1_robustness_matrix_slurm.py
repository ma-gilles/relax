...
def test_autonomous_trajectory_uses_only_iter0_boundary_and_never_emits_replay_logic(tmp_path):
    ...
    assert "RELAX_FINAL_ALL_DATA_REPLAY_LAST_NUMBERED_STATE=0" in submission
...
def test_current_size_oracle_is_explicit_and_auditable(tmp_path):
    ...
    proc, scratch = _dry_run_launcher(
        ...
        extra_env={
            ...
            "RELAX_RELION_CURRENT_SIZES": schedule,
        },
    )
    ...
    assert f"export RELAX_RELION_CURRENT_SIZES={escaped_schedule}" in script
    assert 'RELAX_EXTRA_ARGS+=(--relion_current_sizes "${RELAX_RELION_CURRENT_SIZES}")' in script
    assert f"RELAX_RELION_CURRENT_SIZES={schedule}" in submission
...
def test_scorecard_mode_forces_complete_frozen_suite_evidence_contract(tmp_path):
    ...
    assert "EM_K1_MATRIX_TRAJECTORY_MODE=autonomous" in submission
    assert "RELAX_SAVE_INTERMEDIATES_DIR=1" in submission
    assert "RELAX_SAVE_INTERMEDIATES_SKIP_UNREGULARIZED=1" in submission
    assert "RELAX_FINAL_ALL_DATA_REPLAY_LAST_NUMBERED_STATE=0" in submission
...
def test_scorecard_mode_rejects_legacy_k1_translation_grid(tmp_path):
    ...
    assert proc.returncode == 2
    assert "RELAX_K1_RELION_EXACT_TRANSLATION_GRID unset/on" in proc.stdout
...
def test_exact_local_contribution_capture_is_scoped_and_freezes_metadata(tmp_path):
    ...
    proc, scratch = _dry_run_launcher(
        ...
        case="26",
        extra_env={
            "RELAX_BPREF_CONTRIBUTION_DUMP_DIR": str(dump_dir),
            "RELAX_BPREF_CONTRIBUTION_DUMP_ITERATION": "7",
            "RELAX_BPREF_CONTRIBUTION_DUMP_HALF": "2",
            "RELAX_BPREF_CONTRIBUTION_DUMP_CURRENT_SIZE": "60",
            "RELAX_BPREF_CONTRIBUTION_DUMP_RUN_ID": "case26-it7-h2",
        },
    )
    ...
    assert "export RELAX_BPREF_CONTRIBUTION_DUMP_ITERATION=7" in script
    assert "export RELAX_BPREF_CONTRIBUTION_DUMP_HALF=2" in script
    assert "export RELAX_BPREF_CONTRIBUTION_DUMP_CURRENT_SIZE=60" in script
    assert "RELAX_BPREF_CONTRIBUTION_DUMP_ITERATION=7" in submission
    assert "RELAX_BPREF_CONTRIBUTION_DUMP_HALF=2" in submission
    assert "RELAX_BPREF_CONTRIBUTION_DUMP_CURRENT_SIZE=60" in submission
...
def test_case_jobs_reuse_setup_relion_binding_build_dir(tmp_path):
    ...
    assert '"${PIXI_PY}" relax/relion_bind/build.py' in setup_text
    ...
    assert "relax/relion_bind/build.py" not in case_text
...
def test_queued_jobs_unset_inherited_non_matrix_refinement_overrides(tmp_path):
    inherited_overrides = {
        "RELAX_FINAL_ALL_DATA_AFTER_MAX_ITER": "1",
        "RELAX_DISABLE_RELION_EXACT_FINE_GAUSSIAN": "1",
        "RELAX_USE_FLOAT64_SCORING": "1",
        "RELAX_USE_FLOAT64_PROJECTIONS": "1",
    }
    ...
    for text in job_texts:
        ...
        assert "unset RELAX_USE_FLOAT64_SCORING RELAX_USE_FLOAT64_PROJECTIONS" in text
...
def test_fused_fine_diff2_override_is_recorded_and_replayed(tmp_path):
    proc, scratch = _dry_run_launcher(
        tmp_path,
        case="26",
        extra_env={"RELAX_RELION_FINE_DIFF2_FUSED_FFI": "1"},
    )
    ...
    assert len(job_texts) == 3
    for text in job_texts:
        assert "unset RELAX_RELION_FINE_DIFF2_FUSED_FFI" in text
        assert "export RELAX_RELION_FINE_DIFF2_FUSED_FFI=1" in text
    assert "RELAX_RELION_FINE_DIFF2_FUSED_FFI=1\n" in (
...
def test_bpref_and_relion_mstep_diagnostic_dump_dirs_are_case_scoped(tmp_path):
    ...
    proc, scratch = _dry_run_launcher(
        ...
        case="11",
        extra_env={
            "RELAX_BPREF_ACCUM_DUMP_DIR": str(bpref_root),
            "RELAX_PASS2_DUMP_DIR": str(pass2_root),
            "RELAX_PASS2_DUMP_ORIGINAL_INDICES": "7,19",
            "RELAX_PASS2_DUMP_CURRENT_SIZE": "56",
            "RELAX_MSTEP_DUMP_DIR": str(relion_root),
            "RELAX_MSTEP_DUMP_MAX_CALLS": "6",
            "RELAX_MSTEP_DUMP_RAW": "1",
        },
    )
    ...
    assert f"export RELAX_BPREF_ACCUM_DUMP_DIR={bpref_root}" in text
    assert f"export RELAX_PASS2_DUMP_DIR={pass2_root}" in text
    assert "export RELAX_PASS2_DUMP_ORIGINAL_INDICES=7\\,19" in text
    assert "export RELAX_PASS2_DUMP_CURRENT_SIZE=56" in text
    assert f"export RELAX_MSTEP_DUMP_DIR={relion_root}" in text
    assert "export RELAX_MSTEP_DUMP_MAX_CALLS=6" in text
    assert "export RELAX_MSTEP_DUMP_RAW=1" in text
    assert f'export RELAX_BPREF_ACCUM_DUMP_DIR="${{RELAX_BPREF_ACCUM_DUMP_DIR%/}}/{case_name}"' in text
    assert f'export RELAX_PASS2_DUMP_DIR="${{RELAX_PASS2_DUMP_DIR%/}}/{case_name}"' in text
    assert f'export RELAX_MSTEP_DUMP_DIR="${{RELAX_MSTEP_DUMP_DIR%/}}/{case_name}"' in text
    assert 'mkdir -p "${RELAX_BPREF_ACCUM_DUMP_DIR}"' in text
    assert 'mkdir -p "${RELAX_PASS2_DUMP_DIR}"' in text
    assert 'mkdir -p "${RELAX_MSTEP_DUMP_DIR}"' in text
    submission = (scratch / "submission.env").read_text()
    assert f"RELAX_BPREF_ACCUM_DUMP_DIR={bpref_root}" in submission
    assert f"RELAX_PASS2_DUMP_DIR={pass2_root}" in submission
    assert "RELAX_PASS2_DUMP_ORIGINAL_INDICES=7,19" in submission
    assert "RELAX_PASS2_DUMP_CURRENT_SIZE=56" in submission
    assert f"RELAX_MSTEP_DUMP_DIR={relion_root}" in submission
    assert "RELAX_MSTEP_DUMP_MAX_CALLS=6" in submission
    assert "RELAX_MSTEP_DUMP_RAW=1" in submission
...
def test_local_fused_posterior_diagnostic_dump_dir_is_forwarded_and_case_scoped(tmp_path):
    ...
    proc, scratch = _dry_run_launcher(
        ...
        case="11",
        extra_env={
            "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_DIR": str(dump_root),
            "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_GLOBAL_INDICES": "428,2814",
            "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_CURRENT_SIZE": "128",
            "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_ITERATION": "11",
            "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_LABEL": "final_probe",
        },
    )
    ...
    text = scripts[0].read_text()
    case_name = "11_small_baseline_3k_g128_white_noise1_bf80"
    assert f"export RELAX_LOCAL_FUSED_POSTERIOR_DUMP_DIR={dump_root}" in text
    assert "export RELAX_LOCAL_FUSED_POSTERIOR_DUMP_GLOBAL_INDICES=428\\,2814" in text
    assert "export RELAX_LOCAL_FUSED_POSTERIOR_DUMP_CURRENT_SIZE=128" in text
    assert "export RELAX_LOCAL_FUSED_POSTERIOR_DUMP_ITERATION=11" in text
    assert "export RELAX_LOCAL_FUSED_POSTERIOR_DUMP_LABEL=final_probe" in text
    assert (
        'export RELAX_LOCAL_FUSED_POSTERIOR_DUMP_DIR="${RELAX_LOCAL_FUSED_POSTERIOR_DUMP_DIR%/}/'
        f'{case_name}"'
    ) in text
    assert 'mkdir -p "${RELAX_LOCAL_FUSED_POSTERIOR_DUMP_DIR}"' in text
    submission = (scratch / "submission.env").read_text()
    assert f"RELAX_LOCAL_FUSED_POSTERIOR_DUMP_DIR={dump_root}" in submission
    assert "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_GLOBAL_INDICES=428,2814" in submission
    assert "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_CURRENT_SIZE=128" in submission
    assert "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_ITERATION=11" in submission
    assert "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_LABEL=final_probe" in submission
...
def test_relion_projector_diagnostic_dump_dir_is_forwarded_and_case_scoped(tmp_path):
    ...
    proc, scratch = _dry_run_launcher(
        tmp_path,
        case="22",
        extra_env={"RELAX_RELION_PROJECTOR_DUMP_DIR": str(dump_root)},
    )
    ...
    assert f"export RELAX_RELION_PROJECTOR_DUMP_DIR={dump_root}" in text
    assert (
        'export RELAX_RELION_PROJECTOR_DUMP_DIR="${RELAX_RELION_PROJECTOR_DUMP_DIR%/}/'
        f'{case_name}"'
    ) in text
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("32", "RELAX_K1_RELION_X_HALF_MSTEP", "0"),
        ("26", "RELAX_RELION_X_HALF_MSTEP_DOUBLE", "1"),
    ],
)
...
def test_exact_local_packed_noise_chunk_target_env_is_forwarded(tmp_path):
    proc, scratch = _dry_run_launcher(
        ...
        case="26",
        extra_env={
            "RELAX_EXACT_LOCAL_PACKED_NOISE_TARGET_ROW_PIXELS": "16000000",
            "RELAX_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS": "32000000",
            "RELAX_EXACT_LOCAL_PROGRESS_CHUNKS": "250",
            "RELAX_EXACT_LOCAL_PROGRESS_SECONDS": "90",
        },
    )
    ...
    assert len(scripts) == 1
    text = scripts[0].read_text()
    assert "export RELAX_EXACT_LOCAL_PACKED_NOISE_TARGET_ROW_PIXELS=16000000" in text
    assert "RELAX_EXACT_LOCAL_PACKED_NOISE_TARGET_ROW_PIXELS=16000000" in (
        scratch / "submission.env"
    ).read_text()
    assert "export RELAX_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS=32000000" in text
    assert "RELAX_EXACT_LOCAL_XHALF_PROJECTION_TARGET_ROW_PIXELS=32000000" in (
        scratch / "submission.env"
    ).read_text()
    assert "export RELAX_EXACT_LOCAL_PROGRESS_CHUNKS=250" in text
    assert "export RELAX_EXACT_LOCAL_PROGRESS_SECONDS=90" in text
    submission = (scratch / "submission.env").read_text()
    assert "RELAX_EXACT_LOCAL_PROGRESS_CHUNKS=250" in submission
    assert "RELAX_EXACT_LOCAL_PROGRESS_SECONDS=90" in submission


def test_sparse_pass2_memory_cap_envs_are_forwarded(tmp_path):
    extra_env = {
        "RELAX_SPARSE_PASS2_MAX_TRANSLATION_TILE_BYTES": "123456",
        "RELAX_SPARSE_PASS2_MAX_PROJECTION_GATHER_BYTES": "234567",
        "RELAX_SPARSE_PASS2_MAX_NOISE_BLOCK_BYTES": "345678",
        "RELAX_SPARSE_PASS2_MAX_ADJOINT_BLOCK_BYTES": "456789",
        "RELAX_SPARSE_PASS2_PROJECTION_CACHE_MAX_BYTES": "567890",
        "RELAX_SPARSE_PASS2_MAX_PROJECTED_ROTATIONS": "64",
    }
...
def test_significance_dump_envs_are_forwarded_and_scoped_per_case(tmp_path):
    ...
    extra_env = {
        "RELAX_SIGNIFICANCE_DUMP_DIR": str(dump_root),
        "RELAX_SIGNIFICANCE_DUMP_ORIGINAL_INDICES": "2767",
        "RELAX_SIGNIFICANCE_DUMP_CURRENT_SIZE": "56",
    ...
    for name, value in extra_env.items():
        ...
        assert f"{name}={value}" in submission
    assert (
        'export RELAX_SIGNIFICANCE_DUMP_DIR="${RELAX_SIGNIFICANCE_DUMP_DIR%/}/24_small_kent_outliers_3k_g128_pct20_noise3_bf80"'
        in text
    )
...
def test_local_adaptive_pass2_defaults_to_relion_pruned_parent_in_jobs(tmp_path):
    ...
    text = scripts[0].read_text()
    submission = (scratch / "submission.env").read_text()
    assert "export RELAX_LOCAL_ADAPTIVE_PASS2_FULL_PARENT=0" in text
    assert "RELAX_LOCAL_ADAPTIVE_PASS2_FULL_PARENT=0" in submission
...
def test_k1_batch_defaults_are_forwarded_to_jobs_and_provenance(tmp_path):
    proc, scratch = _dry_run_launcher(
        tmp_path,
        case="32",
        extra_env={"RELAX_RELION_FIRSTITER_RECON_COMPLEX_BUDGET": "805306368"},
    )
    ...
    assert "export RELAX_RELION_EM_BATCH_PROJECTION_FRACTION=0.40" in text
    assert "RELAX_RELION_EM_BATCH_PROJECTION_FRACTION=0.40" in submission
    assert "export RELAX_RELION_FIRSTITER_RECON_COMPLEX_BUDGET=805306368" in text
    assert "RELAX_RELION_FIRSTITER_RECON_COMPLEX_BUDGET=805306368" in submission
...
def test_exact_local_auto_microbatch_boost_is_forwarded(tmp_path):
    proc, scratch = _dry_run_launcher(
        tmp_path,
        case="32",
        extra_env={"RELAX_EXACT_LOCAL_AUTO_MICROBATCH_BOOST": "3"},
    )
    ...
    text = scripts[0].read_text()
    submission = (scratch / "submission.env").read_text()
    assert "export RELAX_EXACT_LOCAL_AUTO_MICROBATCH_BOOST=3" in text
    assert "RELAX_EXACT_LOCAL_AUTO_MICROBATCH_BOOST=3" in submission
...
def test_save_intermediates_skip_unregularized_is_forwarded(tmp_path):
    proc, scratch = _dry_run_launcher(
        ...
        case="32",
        extra_env={
            "RELAX_SAVE_INTERMEDIATES_DIR": "auto",
        },
    )
    ...
    text = scripts[0].read_text()
    submission = (scratch / "submission.env").read_text()
    assert 'export RELAX_SAVE_INTERMEDIATES_SKIP_UNREGULARIZED="1"' in text
    assert "RELAX_SAVE_INTERMEDIATES_SKIP_UNREGULARIZED=1" in submission
...
def test_save_intermediates_unregularized_maps_can_be_reenabled(tmp_path):
    proc, scratch = _dry_run_launcher(
        ...
        extra_env={
            "RELAX_SAVE_INTERMEDIATES_DIR": "auto",
            "RELAX_SAVE_INTERMEDIATES_SKIP_UNREGULARIZED": "0",
        },
    )
    ...
    text = scripts[0].read_text()
    submission = (scratch / "submission.env").read_text()
    assert 'export RELAX_SAVE_INTERMEDIATES_SKIP_UNREGULARIZED="0"' in text
    assert "RELAX_SAVE_INTERMEDIATES_SKIP_UNREGULARIZED=0" in submission
    assert '--save_intermediates_dir "${RELAX_INTERMEDIATES_DIR}"' in text
    assert "--save_intermediates_skip_unregularized" in text
    assert 'if [[ "${RELAX_SAVE_INTERMEDIATES_SKIP_UNREGULARIZED:-}" =~ ^(1|true|TRUE|yes|YES|on|ON)$ ]]' in text
