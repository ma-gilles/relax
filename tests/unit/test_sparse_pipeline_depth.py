...
def test_depth_policy(monkeypatch, value, expected):
    monkeypatch.delenv('RELAX_SPARSE_KCLASS_PIPELINE_DEPTH', raising=False)
    if value is not None:
        monkeypatch.setenv('RELAX_SPARSE_KCLASS_PIPELINE_DEPTH', value)
...
def test_throttle_executes_and_preserves_outputs(monkeypatch, noise):
    ...
    monkeypatch.setenv('RELAX_SPARSE_KCLASS_PIPELINE_DEPTH', '0')
    ...
    waits.clear()
    monkeypatch.setenv('RELAX_SPARSE_KCLASS_PIPELINE_DEPTH', '1')
