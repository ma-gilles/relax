...
from relax.scoring import coarse_partition as p
...
def test_one_overflow_does_not_promote_other_images_or_padding():
    ...
    assert_matches(selected.image_indices, [0, 2, 3])
    assert_matches(full.image_indices, [1])
    assert selected.physical_count == full.physical_count == 32
    assert full.selection is None
    assert_matches(selected.selection.block_count[:3], [1, 64, 2])
    for row, count in enumerate((1, 64, 2)):
        assert_matches(selected.selection.block_ids[row, :count], np.arange(count))
...
def test_homogeneous_batch_preserves_original_physical_shape(counts, selected):
    ...
    assert_matches(group.image_indices, [0, 1, 2])
...
def test_invalid_certificate_keeps_entire_batch_full(field):
    ...
    assert_matches(result.groups[0].image_indices, [0, 1, 2])
...
def test_device_gather_retains_image_order_and_zeroes_poisoned_padding(has_prior):
    ...
    for source, got in zip((shifted, weight, initial, prior), result, strict=True):
        if source is None:
            assert got is None
            continue
        assert_matches(got[:2], source[rows])
        assert_matches(got[2:], np.zeros_like(np.asarray(got[2:])))
...
def test_composition_certifies_once_and_only_full_scores_required_groups(monkeypatch, invalid_selected, capture):
    ...
    from relax.scoring import significance as s
    ...
    def update(value, projected, images, **kwargs):
        ...
        assert_matches(kwargs["translation_log_prior"], prior)
        events.append("certificate")
        return state

    def selected(projected, x, w, i, ids, **kwargs):
        assert_matches(x[:3], shifted[[0, 2, 3]])
    ...
    def full(projected, x, w, i, **kwargs):
        ...
        assert_matches(kwargs["translation_log_prior"][:n], prior[np.asarray(i[:n], np.int32)])
    ...
    assert_matches(groups[0].image_indices, [0, 2, 3])
    assert_matches(groups[1].image_indices, [1])
    ...
    if invalid_selected:
        ...
        assert_matches(groups[0].result.compact_scores.block_count[:3], [1, 64, 2])
