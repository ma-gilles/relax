...
def repo_subprocess_env(env=None):
    """Environment for a child interpreter that must import this checkout's ``relax``.
...
# Exit status of a child whose ``relax`` import resolved outside the repo root.
...
# REPO_IMPORT_ROOT_FAILURE_STATUS unless ``relax`` was imported from under the
...
try:
    ...
    if origin is None or not origin.is_relative_to(root):
        sys.stdout.flush()
        sys.stderr.write(f"child imported relax from {{origin}}, not from under {{root}}\\n")
        sys.stderr.flush()
        os._exit({REPO_IMPORT_ROOT_FAILURE_STATUS})
    pinned = os.environ.get("RELAX_EXPECTED_RECOVAR_ROOT")
    module = sys.modules.get("recovar")
    if pinned and module is not None:
        origin = pathlib.Path(module.__file__).resolve()
        if not origin.is_relative_to(pathlib.Path(pinned).resolve()):
            sys.stdout.flush()
            sys.stderr.write(f"child imported recovar from {{origin}}, not from under {{pinned}}\\n")
            sys.stderr.flush()
            os._exit({REPO_IMPORT_ROOT_FAILURE_STATUS})
"""
...
def repo_python_command(*args):
    """``python *args`` for a child that must fail unless it imported this checkout's ``relax``.

    ``args`` are what would follow ``python``: ``"-c", code, ...``,
    ``"-m", module, ...`` or ``script, ...``. The child runs them unchanged and
    then exits with ``REPO_IMPORT_ROOT_FAILURE_STATUS`` if ``relax`` was not
