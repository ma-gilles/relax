...
`iorientclass = iclass * nr_dir * nr_psi + iorient` (ml_optimiser.cpp:7148, :7192). One unit's cells are one
...
img_id)`, acc_ml_optimiser_impl.h:573, :1089-1091), the per-image CTF and dose, the optics group and the noise
...
sum order matters for parity). For tomography `t` is a 3D translation of the unit (ml_optimiser.cpp:2356, :5857);
each image applies its projected 2D shift `L_i t`, with no rounding of the old offset (ml_optimiser.cpp:6085,
:6115), so the per-image phases are a scoring-side function of `(i, t)`. The sum over images comes before the
per-unit minimum, the weights and the significance (acc_ml_optimiser_impl.h:1075-1136). The interface between
...
- **Statistics.**
  ...
  - Noise is one total over classes per optics group (ml_optimiser.cpp:8638, :9106). The scoring side owns
    the per-image indexing of the noise and power sums; the posterior side supplies only `posterior(h, t)`. The
    per-particle divisions of tomography are cryoet's (acc_ml_optimiser_impl.h:2840-2846).
