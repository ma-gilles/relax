    ...
    for k in range(2):
        ...
        if tomo_halves:
            ...
            final_result = _score_tomo_half_in_loop(
                ...
                sampling=TomoSampling(
                    healpix_order=int(final_local_search_order) - final_tomo_oversampling,
                    oversampling_order=final_tomo_oversampling,
                    offset_range_angst=float(final_translation_range) * float(cryo.voxel_size),
                    offset_step_angst=float(final_translation_step) * float(cryo.voxel_size),
                    random_perturbation=float(final_local_search_random_perturbation),
                    coarse_size=int(final_local_pass1_current_size),
                    fine_size=int(final_current_size),
                ),
                local_search=dict(
                    previous_eulers_deg=relion_half_inputs.previous_best_rotation_eulers[k],
