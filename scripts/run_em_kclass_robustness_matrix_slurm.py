...
# Slurm memory per case is sized from measured peak RSS of the case jobs (sacct MaxRSS of the
# em_kcls_* runs, 2026-04 to 09): every case peaked at 8-33 GB except case 5 (K4 50k/256, 90 GB)
# and case 6 (K4 50k/256 radial, 133 GB), so 128G each and 180G for case 6.
...
def build_cuda_lib_command() -> str:
    return """mkdir -p "$(dirname "${RELAX_CUDA_LIB}")"
CUDA_LIB_TMP="${RELAX_CUDA_LIB}.${SLURM_JOB_ID:-$$}.tmp"
export CUDA_LIB_TMP PIXI_PY
flock "$(dirname "${RELAX_CUDA_LIB}")/build.lock" bash -lc '
  set -euo pipefail
  if [[ -s "${RELAX_CUDA_LIB}" && -s "${RELAX_CUDA_LIB}.sha256" ]]; then
    sha256sum --check "${RELAX_CUDA_LIB}.sha256"
    echo "Reusing sealed CUDA library ${RELAX_CUDA_LIB}"
    exit 0
  fi
  rm -f "${CUDA_LIB_TMP}"
  env PYTHON="${PIXI_PY}" make -C relax/cuda LIB="${CUDA_LIB_TMP}" all
  mv -f "${CUDA_LIB_TMP}" "${RELAX_CUDA_LIB}"
  sha256sum "${RELAX_CUDA_LIB}" > "${RELAX_CUDA_LIB}.sha256"
'
sha256sum --check "${RELAX_CUDA_LIB}.sha256"
"""
...
def verify_cuda_lib_command() -> str:
    return """if [[ ! -s "${RELAX_CUDA_LIB}" || ! -s "${RELAX_CUDA_LIB}.sha256" ]]; then
  echo "ERROR: setup did not seal the shared CUDA library: ${RELAX_CUDA_LIB}" >&2
  exit 2
fi
sha256sum --check "${RELAX_CUDA_LIB}.sha256"
"""
...
export RELAX_CUDA_LIB={q(cuda_lib)}
...
mkdir -p "${{TMPDIR}}" "${{PIXI_HOME}}" "${{RATTLER_CACHE_DIR}}" "${{RECOVAR_JAX_CACHE_DIR}}" "${{RECOVAR_CUDA_CACHE_DIR}}" "$(dirname "${{RELAX_CUDA_LIB}}")"
    ...
    (scratch_dir / "submission.env").write_text(
        "\n".join(
            [
                ...
                f"RELAX_CUDA_LIB={shared_cuda_lib}",
