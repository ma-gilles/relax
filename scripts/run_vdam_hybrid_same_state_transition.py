...
COMPACT_POSTERIOR_ENVIRONMENT = "RELAX_COARSE_GAUSSIAN_GEMM_COMPACT_POSTERIOR"
FLAT_ROW_ENVIRONMENT = "RECOVAR_INITIAL_MODEL_FLAT_LOCAL_ROWS"
STABLE_FLAT_CAPACITY_ENVIRONMENT = (
    "RELAX_INITIAL_MODEL_STABLE_FLAT_ROW_CAPACITY"
)
...
PACKED_FINAL_NOISE_ENVIRONMENT = "RELAX_INITIAL_MODEL_PACKED_FINAL_NOISE"
...
FUSED_PAIR_FINE_SCORE_ENVIRONMENT = "RELAX_EXACT_LOCAL_FUSED_PAIR_FINE_SCORE"
...
BATCHED_POSTERIOR_ENVIRONMENT = "RELAX_RELION_BATCHED_POSTERIOR_PRIMITIVES"
STABLE_FOURIER_QUANTUM_ENVIRONMENT = (
    "RELAX_RELION_VDAM_STABLE_FOURIER_WINDOW_QUANTUM"
)
GPU_MONITOR_ENVIRONMENT = "RELAX_SAME_STATE_GPU_MONITOR_CSV"
...
def _validate_fused_coarse_projector_profiles(
    ...
    *,
    ...
    from relax.diagnostics.coarse_score_diagnostics import _validate_coarse_selector_audit
...
def _run_transition_arm(
    ...
    *,
    ...
    import relax.vdam.driver as driver
    from recovar.data_io.starfile import read_star
    from relax.vdam import m_step, native_sampling
    ...
    if fused_posterior_dump_original_index is not None:
        diagnostic_environment = {
            "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_DIR": str(
                checkpoint["output_root"] / "fused_posterior_dumps"
            ),
            "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_GLOBAL_INDICES": str(
                int(fused_posterior_dump_original_index)
            ),
            "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_ITERATION": str(
                int(checkpoint_iteration) + 1
            ),
            "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_LABEL": label,
            "RELAX_LOCAL_FUSED_POSTERIOR_DUMP_SCORES": "1",
        }
    if coarse_prefix_dump_original_index is not None:
        diagnostic_environment.update(
            {
                "RELAX_COARSE_RUNTIME_PREFIX_DUMP_DIR": str(
                    checkpoint["output_root"] / "coarse_prefix_dumps"
                ),
                "RELAX_COARSE_RUNTIME_PREFIX_DUMP_ORIGINAL_INDICES": str(
                    int(coarse_prefix_dump_original_index)
                ),
                "RELAX_COARSE_RUNTIME_PREFIX_DUMP_LABEL": label,
            },
        )
